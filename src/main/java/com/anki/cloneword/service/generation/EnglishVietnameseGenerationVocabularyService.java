...
@Service(value = "engVieGeneration")
public class EnglishVietnameseGenerationVocabularyService extends GenerationVocabularyService {
    ...
    private final String PATTERN_MEAN =
            ...
            "            <p style=\"font-weight: bold; color: rgb(0,221,162)\">``trans``</p>\n";
    ...
    public String generate(WordDescription wordDescription) {
        ...
        if (!CollectionUtils.isEmpty(wordDescription.getEntries())) {
            builder.append("<div style=\"font-size: 15px; text-align: left;\">\n    <p style=\"color: rgb(224,102,102); font-weight: bold; font-size: 25px;\">English - Vietnamese Dictionary</p>\n    <div>\n");
            for (WordDescription.EntryDescription entry : wordDescription.getEntries()) {
                ...
                String wordDef = PATTERN_WORD_DEFINE.replace("``word``", entry.getWord())
                        ...
                        .replace("``audioUK``", StringUtils.trim(entry.getAudioUK()))
                        .replace("``pronounceUK``", StringUtils.trim(entry.getPronounceUK()))
                        .replace("``audioUS``", StringUtils.trim(entry.getAudioUS()))
                        .replace("``pronounceUS``", StringUtils.trim(entry.getPronounceUS()));
